...
export const FarmProvider: React.FC<{ children: React.ReactNode }> = ({ children }) => {
  ...
  const loadAnimals = useCallback(async (force = false) => {
    ...
    try {
      // Query lightweight animal fields with primary_photo only
      const { data, error } = await supabase
        .from("animals")
  ...
  const addAnimal = async (animalData: Omit<Animal, "id" | "animal_code" | "created_at">) => {
    ...
    const speciesCode = get<dyad-write path="src/context/FarmContext.tsx" description="Complete implementation of FarmContext with lightweight roster querying, primary photo loading, and purchase price support">
"use client";
