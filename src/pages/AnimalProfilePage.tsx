...
export const AnimalProfilePage: React.FC = () => {
  ...
  // Edit Animal Form state & custom species state
  const [editSpeciesOption, setEditSpeciesOption] = useState<string>("Goat");
  const [editCustomSpecies, setEditCustomSpecies] = useState<string>("");
  ...
  // Offspring creation state & custom species
  const [offspringSpeciesOption, setOffspringSpeciesOption] = useState<string>(animal ? animal.species : "Goat");
  const [offspringCustomSpecies, setOffspringCustomSpecies] = useState<string>("");
  ...
  useEffect(() => {
    if (animal) {
      const isKnown = SPECIES_OPTIONS.includes(animal.species as any);
      setEditSpeciesOption(isKnown ? animal.species : "Other");
      setEditCustomSpecies(isKnown ? "" : animal.species);
      ...
      setEditForm({
        ...
        dob: animal.dob || "",
        purchaseDate: animal.purchaseDate || "",
        purchasePrice: animal.purchasePrice !== undefined && animal.purchasePrice !== null ? String(animal.<dyad-write path="src/pages/AnimalProfilePage.tsx" description="Complete implementation of AnimalProfilePage with acquisition details, purchase price support, and image gallery">
"use client";
...
export const AnimalProfilePage: React.FC = () => {
  ...
  useEffect(() => {
    if (animal) {
      ...
      const isOffspringKnown = SPECIES_OPTIONS.includes(animal.species as any);
      setOffspringSpeciesOption(isOffspringKnown ? animal.species : "Other");
      setOffspringCustomSpecies(isOffspringKnown ? "" : animal.species);
  ...
  const handlePhotoUpload = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (files && files.length > 0) {
      try {
        ...
        const updatedPhotos = [...(animal.photos || []), ...uploadedList];
        const primary = animal.primaryPhoto || uploadedList[0];
        await updateAnimal(animal.id, { photos: updatedPhotos, primaryPhoto: primary });
        showSuccess(`${uploadedList.length} photo${uploadedList.length > 1 ? "s" : ""} added!`);
      } catch (err) {
      ...
      <html>
        ...
        <body>
          ...
          <table>
            ...
            <tr><th>Purchase Date</th><td>${animal.purchaseDate || "N/A"}</td></tr>
            <tr><th>Purchase Price</th><td>${animal.purchasePrice !== undefined && animal.purchasePrice !== null ? `₦${animal.purchasePrice.toLocaleString()}` : "N/A"}</td></tr>
  ...
  const finalEditSpecies = editSpeciesOption === "Other"
    ? (editCustomSpecies.trim() || "Other")
    : editSpeciesOption;

  const triggerEditConfirm = (e: React.FormEvent) => {
    e.preventDefault();

    if (editSpeciesOption === "Other" && !editCustomSpecies.trim()) {
      showError("Please enter custom species name.");
      return;
    }
    ...
    setPendingConfirm({
      ...
      onConfirm: () => {
        const parsedPrice = editForm.source === "Purchased" && editForm.purchasePrice && !isNaN(Number(editForm.purchasePrice))
          ? parseFloat(editForm.purchasePrice)
          : undefined;

        updateAnimal(animal.id, {
          name: editForm.name,
          species: finalEditSpecies,
          breed: editForm.breed,
          sex: editForm.sex,
          dob: editForm.dob,
          purchaseDate: editForm.source === "Purchased" ? (editForm.purchaseDate || undefined) : undefined,
  ...
  const finalOffspringSpecies = offspringSpeciesOption === "Other"
    ? (offspringCustomSpecies.trim() || "Other")
    : offspringSpeciesOption;

  const triggerOffspringConfirm = (e: React.FormEvent) => {
    e.preventDefault();

    if (offspringSpeciesOption === "Other" && !offspringCustomSpecies.trim()) {
      showError("Please enter custom species name.");
      return;
    }
    ...
    setPendingConfirm({
      ...
      message: `Register offspring (${finalOffspringSpecies}) and link to ${displayName}?`,
      onConfirm: () => {
  ...
  const eligibleMothers = animals.filter(a => a.id !== animal.id && a.sex === "Female");
  const eligibleFathers = animals.filter(a => a.id !== animal.id && a.sex === "Male");
    ...
    <div className="min-h-screen bg-[#FBFDF9] text-slate-800 pb-20 md:pb-8">
      ...
      <div className="max-w-5xl mx-auto p-4 md:p-8 space-y-6">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
          ...
          {/* COLUMN 1: HERO PORTRAIT & ACQUISITION & OWNERSHIP CARD */}
          <div className="md:col-span-1 space-y-4">
            ...
            <div className="bg-white rounded-3xl overflow-hidden border border-slate-100 shadow-sm relative">
              ...
              >
                ...
                <div className="absolute inset-0 bg-black/30 opacity-0 group-hover:opacity-100 flex items-center justify-center text-white transition text-xs font-black gap-2 z-10">
            ...
            <div className="bg-white rounded-3xl p-5 border border-slate-100 shadow-sm space-y-3">
              ...
              <div className="space-y-2 text-xs pt-1">
                <div className="flex items-center justify-between p-2.5 bg-slate-50 rounded-xl">
                  <span className="text-[10px] font-bold text-slate-400 uppercase">Acquisition Source</span>
                ...
                <div className="flex items-center justify-between p-2.5 bg-slate-50 rounded-xl">
                  ...
                  <span className="font-bold text-slate-800">
                    {animal.source === "Born on farm" ? "N/A (Born on farm)" : (animal.purchaseDate || "Unrecorded")}
                  </span>
                </div>
                ...
                <div className="flex items-center justify-between p-2.5 bg-slate-50 rounded-xl">
                  ...
                  <span className="font-bold text-slate-800">
                    {animal.source === "Born on farm" ? (
                      <span className="text-slate-400 font-medium">N/A (Born on farm)</span>
                    ) : animal.purchasePrice !== undefined && animal.purchasePrice !== null ? (
                      <span className="font-black text-emerald-900">₦{animal.purchasePrice.toLocaleString()}</span>
                    ) : (
                  ...
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                    ...
                    >
                      <div className="w-10 h-10 rounded-xl bg-slate-200 overflow-hidden shrink-0 flex items-center justify-center">
                        {mother ? (
                          <LazyAnimalImage src={mother.primaryPhoto} alt={mother.name || mother.animal_code} />
                        ) : (
                    ...
                    >
                      <div className="w-10 h-10 rounded-xl bg-slate-200 overflow-hidden shrink-0 flex items-center justify-center">
                        {father ? (
                          <LazyAnimalImage src={father.primaryPhoto} alt={father.name || father.animal_code} />
                        ) : (
                    ...
                    {offspringList.map(child => (
                      ...
                      >
                        <div className="w-10 h-10 rounded-full bg-slate-100 overflow-hidden mx-auto mb-1.5">
                          <LazyAnimalImage src={child.primaryPhoto} alt={child.name || child.animal_code} />
                        </div>
                ...
                <div className="grid grid-cols-2 sm:grid-cols-3 gap-3">
                  {animal.photos.map((ph, idx) => (
                    <div key={idx} className="bg-white border rounded-2xl overflow-hidden shadow-sm flex flex-col group relative">
                      <div className="h-36 w-full cursor-zoom-in" onClick={() => setFullscreenPhoto(ph)}>
                        <LazyAnimalImage src={ph} alt="Animal gallery photo" containerClassName="w-full h-full" />
                      </div>
          ...
          >
            ...
            <div className="grid grid-cols-2 gap-3">
              <div>
                ...
                <select
                  value={editSpeciesOption}
                  onChange={(e) => setEditSpeciesOption(e.target.value)}
            ...
            {/* Custom species text input in edit modal */}
            {editSpeciesOption === "Other" && (
            ...
            <div className="grid grid-cols-2 gap-3 bg-emerald-50/50 p-3 rounded-2xl border border-emerald-100">
              ...
              <div>
                ...
                <input
                  ...
                  className="w-full p-2 bg-white border rounded-xl text-xs font-bold text-slate-800 disabled:opacity-40"
                />
              </div>
            </div>
      ...
      {/* OFFSPRING MODAL */}
      {showAddOffspring && (
        <div className="fixed inset-0 z-50 bg-black/60 backdrop-blur-sm flex items-end sm:items-center justify-center p-4 max-h-[90vh] overflow-y-auto">
          <form 
          ...
          >
            ...
            <div className="grid grid-cols-2 gap-3">
              <div>
                <label className="text-[10px] font-bold text-slate-500 block">Species</label>
                <select
                  value={offspringSpeciesOption}
                  onChange={(e) => setOffspringSpeciesOption(e.target.value)}
                  className="w-full p-2.5 bg-slate-50 border rounded-xl text-xs mt-1 font-semibold"
                >
                  {SPECIES_OPTIONS.map((spec) => (
                    <option key={spec} value={spec}>
            ...
            {/* Custom Species Text Input for Offspring */}
            {offspringSpeciesOption === "Other" && (
              <div className="animate-in fade-in">
                <label className="text-[10px] font-bold text-emerald-800 uppercase block mb-1">
                  Specify Custom Species Name <span className="text-red-500">*</span>
                </label>
                <input
                  type="text"
                  placeholder="e.g. Pigeon, Ostrich, Alpacas..."
                  value={offspringCustomSpecies}
                  onChange={(e) => setOffspringCustomSpecies(e.target.value)}
                  className="w-full p-2.5 bg-emerald-50/50 border border-emerald-300 rounded-xl text-xs font-bold text-slate-900 outline-none focus:ring-1 focus:ring-emerald-500"
                  required
                />
              </div>
